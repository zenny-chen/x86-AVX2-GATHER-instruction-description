...
这里的第三个操作数 **`XMM0`** 表示一个 mask 操作数，用于指定从每个元素所指定的存储器地址带条件地加载并更新到与目的操作数所对应的每个数据元素。这里所谓的“条件”是根据该 mask 寄存器的每个数据元素的最高有效比特位来指定。如果某一元素的 mask 比特位为 **`0`**，那么相应的目的寄存器的数据元素保持不变。目的寄存器与 mask 寄存器中的数据元素位宽都是一样的。这条指令执行之后会将整个 mask 寄存器清零，除非该指令引发了一个异常。
